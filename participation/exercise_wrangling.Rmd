...
gapminder %>%
  ...
  mutate(gdpBill = sprintf("%0.2f", gdpBill))
...
```{r}
gapminder %>%
  transmute(grossGDP = pop*gdpPercap) %>% 
  transmute(gdpBill = grossGDP/1000000000) %>% 
  transmute(gdpBill = sprintf("%0.2f", gdpBill))
```
...
psych::bfi %>%
  ...
  mutate(across(A1:O5, ~ (.x - 1) / 5 * 100)) #.x is placeholder for A1 thru o5
...
gapminder %>% 
  filter(country == "Canada") %>% 
  mutate(across(where(is.numeric), ~ .x - mean(.x, na.rm = TRUE))) #where variables are numeric, do this
```
...
```{r}
#rowwise function will calculate the mean score for each row of data, but still needs to be ungrouped 
