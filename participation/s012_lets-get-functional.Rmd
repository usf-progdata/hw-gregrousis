...
descriptives <- bfi_mean %>% 
    summarize(across(all_of(big_five), list(mean = mean, sd = sd), na.rm = TRUE)) %>% 
    pivot_longer(cols = everything(), names_to = c("scale", ".value"), names_sep = "_")
...
ind_bfi <- function(.dat){
  sample_mean <- .dat %>% 
    slice_head(n = 10) %>% 
    summarize(across(c(A, C, N, E, O),(mean = mean), na.rm = TRUE))
  ...
  return(c(sample_mean, full_mean, f(sample_mean)))
}
...
bfi_mean %>% 
  slice_head(n = 10) %>% 
  summarize(across(c(A, C, N, E, O),list(mean = mean, sd = sd), na.rm = TRUE)) %>% 
  pivot_longer(cols = everything(), names_to = c("scale", ".value"), names_sep = "_")
